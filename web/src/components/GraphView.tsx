...
/** Deterministic color for a group query, so the same query always maps to
 * the same color across vaults/sessions instead of depending on group order. */
function hashGroupColor(label: string): string {
  let h = 0;
  ...
  const setGroup = (i: number, g: Partial<{ query: string; color: string }>) =>
    patch({
      groups: s.groups.map((x, j) =>
        j === i ? { ...x, ...g, ...(g.query ? { color: hashGroupColor(g.query) } : {}) } : x,
      ),
    });
